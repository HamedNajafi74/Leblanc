...
Br=(Bm).*sin(w*t);     %(Bm*R).*sin(w*t)          ;% +     0.10*(Bm*R).*sin(3*w*t);             %Mfield produced by   phase  'R'  color=Black  +   3rd Harmonic
Bs=(Bm/2).*(sin(w*t)+j*cos(w*t));     %(Bm*S).*sin(w*t-2*pi/3)   ;% +     0.15*(Bm*S).*sin(3*(w*t-2*pi/3));    %Mfield produced by   phase  'S'  color=Blue   +   3rd Harmonic
Bt=(Bm/2).*(sin(w*t)-j*cos(w*t));    %(Bm*T).*sin(w*t+2*pi/3)   ;% +     0.12*(Bm*T).*sin(3*(w*t+2*pi/3));    %Mfield produced by   phase  'T'  color=Red    +   3rd Harmonic

%% Mfields in cartesian coordinates:

% [Rx,Ry]=pol2cart(alpha,Br);
%Rx=Br;Ry=Br-Br;
...
for ii=1:n
    ...
    %       hold on;
    plot(c1,'k','LineWidth',0.5);
    hold on;
    plot(c2,'g','LineWidth',0.5);
    %   hold on;
    ...
    %  hold on;
    plot([0 real(Bs(ii))],[0 imag(Bs(ii))],'r','Marker','s','LineWidth',1);
    %  hold on;
    plot([0 real(Bt(ii))],[0 imag(Bt(ii))],'b','Marker','s','LineWidth',1);
    %  hold on;
